...
import com.github.bitc3t.bitapi.config.FileManager;
...
public class TeamRegistry {
    ...
    public TeamRegistry(BitAPI plugin) {
        this.plugin = plugin;
    }
    ...
    public void readPlayers() {
        for(String playerUUID : playerManager.getConfig().getKeys(false)) {
            try {
                ...
                BitPlayer bitPlayer = new BitPlayer(uuid, team, playerScore);

                this.plugin.getTeamController().addPlayer(bitPlayer);
    ...
    public void registerPlayer(UUID uuid, Team team) throws IOException {
        BitPlayer bitPlayer = new BitPlayer(uuid, team, 0);
        bitPlayer.apiSetup();
        ...
        config.set(uuid + ".player-name", bitPlayer.getPlayer().getName());
